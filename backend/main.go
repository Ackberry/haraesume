...
func main() {
	...
	mux.Handle("/api/cover-letter", requireAuth(http.HandlerFunc(rh.GenerateCoverLetter)))
	mux.Handle("/api/generate-application-package", requireAuth(http.HandlerFunc(rh.GenerateApplicationPackage)))
	mux.Handle("/api/generate-cover-letter-pdf", requireAuth(http.HandlerFunc(rh.GenerateCoverLetterPDF)))
